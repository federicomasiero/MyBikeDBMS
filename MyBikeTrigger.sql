...
CREATE FUNCTION checkEnableCard() RETURNS TRIGGER AS $$
	BEGIN
		IF(NEW.enabled = TRUE) THEN
			-- Check if there is another card enabled with same customer_id
			PERFORM customer_id 
				FROM mbt.Card
				WHERE customer_id = NEW.customer_id AND enabled = TRUE;
			IF FOUND THEN
				RAISE EXCEPTION 'Customer % already has an enabled card.', new.customer_id;
			END IF;
			RETURN NEW;
		ELSE
			-- Check if there is another card enabled with same customer_id
...
CREATE TRIGGER checkCard
	...
	FOR EACH ROW
	EXECUTE PROCEDURE checkEnableCard();
